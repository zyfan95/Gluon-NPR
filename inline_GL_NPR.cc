...
*/
    ...
    Complex G2ptnew1(int Mu, multi1d<int> p, LatticeColorMatrix A_x, int rmax1)
    {
        Complex G_2pt=0;
        ColorMatrix  A_p, A_mp;
        ...
        Real p_dot_x=0;
        LatticeComplex AA=0;
        LatticeColorMatrix A_x1, Amid, A_x2,  A_x3,  A_x4, A_x5, A_x6;

	int dx, dy, dz;	

        A_x1=A_x;
        for(int rmax=1; rmax<=rmax1; rmax++)
        {
	A_x1=A_x;
        ...
        A_x1=A_x2;
        p_dot_x=twopi*(1.0*p[0]*dx/Layout::lattSize()[0]+1.0*p[1]*dy/Layout::lattSize()[1]+1.0*p[2]*dz/Layout::lattSize()[2]+1.0*p[3]*(-rmax+1)/Layout::lattSize()[3]);
        ...
        A_x6=A_x5;
        Amid=A_x6;
        A_x6=shift(Amid, FORWARD, 3);

        for(int dt = -rmax+2; dt<= rmax-2; dt++)
        {
        Amid=A_x6;
        A_x6=shift(Amid, FORWARD, 3);
...
/*
        Amid=A_x6;
        A_x6=shift(Amid, BACKWARD, 0);
        Amid=A_x6;
        A_x6=shift(Amid, BACKWARD, 1);
        Amid=A_x6;
        A_x6=shift(Amid, BACKWARD, 2);
*/
        ...
        //QDPIO::cout <<"dG2pt   "<<  G_2pt <<std::endl;
...
//z=-r
        ...
        Amid=A_x3;
        A_x3=shift(Amid, FORWARD, 1);
        for(int dy = -rmax+2; dy <= rmax-2; dy++)
        {
        Amid=A_x3;
        A_x3=shift(Amid, FORWARD, 1);
        A_x2=A_x3;
        A_x1=A_x2;
        p_dot_x=twopi*(1.0*p[0]*dx/Layout::lattSize()[0]+1.0*p[1]*dy/Layout::lattSize()[1]+1.0*p[2]*(-rmax+1)/Layout::lattSize()[2]+1.0*p[3]*dt/Layout::lattSize()[3]);
        AA=cmplx(cos(p_dot_x),sin(p_dot_x))*trace(A_x1*A_x);
...
//x=-r
        ...
        Amid=A_x4;
        A_x4=shift(Amid, FORWARD, 2);
        for(int dz = -rmax+2; dz <= rmax-2; dz++)
        {
        Amid=A_x4;
        A_x4=shift(Amid, FORWARD, 2);
        A_x3=A_x4;
        ...
        A_x2=A_x3;
        A_x1=A_x2;
        p_dot_x=twopi*(1.0*p[0]*(-rmax+1)/Layout::lattSize()[0]+1.0*p[1]*dy/Layout::lattSize()[1]+1.0*p[2]*dz/Layout::lattSize()[2]+1.0*p[3]*dt/Layout::lattSize()[3]);
        AA=cmplx(cos(p_dot_x),sin(p_dot_x))*trace(A_x1*A_x);
...
//y=-r
        A_x4=A_x6;
        Amid=A_x4;
        A_x4=shift(Amid, FORWARD, 1);


        Amid=A_x4;
        A_x4=shift(Amid, FORWARD, 0);
        for(int dx = -rmax+2; dx <= rmax-2; dx++)
        {
        Amid=A_x4;
        A_x4=shift(Amid, FORWARD, 0);
        A_x3=A_x4;
        ...
        {
        Amid=A_x3;
        A_x3=shift(Amid, FORWARD, 2);
        A_x2=A_x3;
        A_x1=A_x2;
        p_dot_x=twopi*(1.0*p[0]*dx/Layout::lattSize()[0]+1.0*p[1]*(-rmax+1)/Layout::lattSize()[1]+1.0*p[2]*dz/Layout::lattSize()[2]+1.0*p[3]*dt/Layout::lattSize()[3]);
        AA=cmplx(cos(p_dot_x),sin(p_dot_x))*trace(A_x1*A_x);
...
//y=r
        ...
        Amid=A_x6;
        A_x6=shift(Amid, FORWARD, 0);
        Amid=A_x6;
        A_x6=shift(Amid, FORWARD, 1);
        Amid=A_x6;
        A_x6=shift(Amid, FORWARD, 2);
	...
	{
        A_x4=A_x5;
        for(int dt = -rmax+1; dt <= rmax-1; dt++)
        {
        Amid=A_x4;
        A_x4=shift(Amid, FORWARD, 3);
        }
    ...
    Complex G3ptnew(LatticeColorMatrix Op, LatticeColorMatrix A_x, int rmax)
    {
        Complex G_2pt;
        ColorMatrix  A_p, A_mp;
        ...
        for(int dt = -33+1; dt < 33-1; dt++)
        {
        Amid=A_x1;
        A_x1=shift(Amid, FORWARD, 3);
        //p_dot_x=twopi*(1.0*p[0]*dx/Layout::lattSize()[0]+1.0*p[1]*dy/Layout::lattSize()[1]+1.0*p[2]*dz/Layout::lattSize()[2]+1.0*p[3]*dt/Layout::lattSize()[3]);
        AA=AA+trace(A_x1)*trace(A_x);
        }
        }
        }
        }
	...
	//QDPIO::cout <<"AA   "<<  sum(cmplx(cos(p_dot_x),sin(p_dot_x))*trace(A_x1*A_x)) <<std::endl;
...
*/
    ...
    LatticeColorMatrix  AP(int Mu, LatticeReal p_dot_x, multi1d<int> p, LatticeColorMatrix A_x)
    {
        Complex G_2pt;
        LatticeColorMatrix  A_p, A_mp;
...
*/
	...
	multi5d<LatticeColorMatrix> A_P;
        A_P.resize(Nd,2*pmax+1,2*pmax+1,2*pmax+1,2*pmax+1);
        ...
        for(int i = -pmax; i < pmax+1; i++)
                for(int j = -pmax; j < pmax+1; j++)
					...
					GL2PT=A_P[mu][i+pmax][j+pmax][k+pmax][l+pmax]*A_P[nu][-i+pmax][-j+pmax][-k+pmax][-l+pmax];
					for(int r = 1; r < 9; r++)
					{	
					GL3pt=trace(G3ptnew(Op[0], GL2PT, r));
					QDPIO::cout <<"Op   "<< trace(sum(Op[0])) <<std::endl;
					QDPIO::cout <<"G2pt2   "<< trace(sum(GL2PT)) <<std::endl;
					QDPIO::cout <<"G3pt   " << r << "  "<< mu << "  " << nu << "  "<< i << "  " << j <<"  "<< k <<"  "<< l <<"  "<< real(GL3pt) << "  " << imag(GL3pt) <<std::endl;
					}
					for(int r = 13; r < 14; r++)
                                        {
                                        GL3pt=trace(G3ptnew(Op[0], GL2PT, r));
